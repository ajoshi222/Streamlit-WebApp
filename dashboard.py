...
with st.sidebar:
    ...
    Age_filter = st.multiselect(
        label="Select Age Group", options=df["age"].unique(), default=df["age"].unique()
    )
...
with Q1:
    ...
    fig_CTR_by_campaign = px.bar(
        df3, x="campaign", y="CTR", title="<b>Click Through Rate</b>"
    )
    fig_CTR_by_campaign.update_layout(
        title={"x": 0.5},
...
with Q2:
    fig_impressions_per_day = px.line(
        df1,
        x="date",
        y=["Impressions"],
        color="campaign",
    ...
    fig_impressions_per_day.update_layout(
...
with Q3:
    ...
    fig_spend_by_gender = px.pie(
        df4, names="gender", values="Spent", title="<b>Ad Spend By Gender</b>"
    )
...
with Q4:
    ...
    fig_CPC_by_age = px.bar(
        df5, x="age", y="CPC", title="<b>Cost Per Conversion By Age Demographic</b>"
    )
    fig_CPC_by_age.update_layout(
        title={"x": 0.5},
        xaxis=(dict(showgrid=False)),
        yaxis=(dict(showgrid=False)),
        plot_bgcolor="rgba(0,0,0,0)",
